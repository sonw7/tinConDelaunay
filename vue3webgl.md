...
借助现代计算机技术，在三角剖分算法中，初始三角形的构建可以采用更加灵活的方式。通过内存指向创建一个足够大的"超级三角形"作为剖分的起点，该三角形由三个虚拟点构成，并确保能够包含所有实际数据点。因此只需从实际数据点中来选取一个简单的初始三角形结构开始剖分。在剖分完成后，我们只需识别并移除包含虚拟点的三角形，即可得到仅基于实际数据点的最终三角网格。这种虚拟点策略避免了在实际点集中寻找合适初始三角形的复杂性，同时确保了剖分结果的质量。
...
我现在有个需求。支持在导入的layer地层中，选择两个，然后计算层间距，由A地层的所有点在垂直方向增加分分量构造一个点入（x,y,z）(x,y,z+1),用该射线B地层所有三角面进行求交，如果有则求点到交点距离并保存。定义一个间距和color到mapping，启用顶点着色。并支持重置原有地层
```
